...
const linkedinIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="-143 145 512 512">
    ...
    c-1.4,3.4-1.2,8.2-1.2,13.1v86.3H71.8c0,0,0.7-146.4,0-159.7h56.1v25.1c3.3-11,21.2-26.6,49.8-26.6c35.5,0,63.3,23,63.3,72.4V508.1z" fill="currentColor"/>
  </svg>
);
...
export default function Footer() {
  ...
  // Categorias de navegação atualizadas
  const categories = [
    {
      title: 'Sobre mim',
      links: [
    ...
    <footer className="w-full bg-[var(--color-bg-secondary)] text-[var(--color-text-secondary)] border-t border-[var(--color-bg-tertiary)] py-10 px-4">
      <div className="container mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 text-center md:text-left">
          {categories.map((cat) => (
            <div key={cat.title}>
              <h4 className="font-semibold text-base text-[var(--color-text-primary)] mb-3">{cat.title}</h4>
          ...
          <div>
            ...
            <div className="flex flex-col space-y-2">
              <a href="mailto:marllon@bazestudio.com.br" className="hover:text-[var(--color-text-accent)] transition">
                marllon@bazestudio.com.br
              </a>
              <a href="https://wa.me/5521987881633" target="_blank" rel="noopener noreferrer" className="hover:text-[var(--color-text-accent)] transition">
                (21) 98788-1633
              </a>
            </div>
            <div className="mt-4">
              ...
              <div className="flex justify-center md:justify-start space-x-5">
                {iconsFooter.map((icon) => (
                  <Link key={icon.name} href={icon.link} target="_blank" rel="noopener noreferrer" className="text-[var(--color-text-secondary)] hover:text-[var(--color-text-accent)] transition">
                    {icon.name === 'Instagram' ? <icon.logo size={24} /> : <icon.logo className="w-6 h-6" />}
                  </Link>
                ))}
              </div>
            </div>
